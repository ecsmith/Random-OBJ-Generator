...
class curveRevolve extends JFrame implements GLEventListener, KeyListener, MouseListener, MouseMotionListener, ActionListener {
	...
	public curveRevolve() {
		super("Assignment 3 -- Curve Revolve");
		canvas = new GLCanvas();
	...
	public static void main(String[] args) {
		...
		vertBuffer = BufferUtil.newFloatBuffer(verts.size()*3);
		normBuffer = BufferUtil.newFloatBuffer(verts.size()*3);
		faceBuffer = BufferUtil.newIntBuffer(faces.size());
	...
	public void display(GLAutoDrawable drawable) {
		...
		gl.glDrawElements(GL.GL_QUADS, faces.size(), GL.GL_UNSIGNED_INT, faceBuffer);
