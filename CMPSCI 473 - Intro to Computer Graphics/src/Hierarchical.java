...
class Hierarchical extends JFrame implements GLEventListener, KeyListener, MouseListener, MouseMotionListener, ActionListener {
	...
	private objModel example_model1 = new objModel("base.obj");
	private objModel example_model2 = new objModel("sculpt1.obj");
	private objModel example_model3 = new objModel("sculpt2.obj");
	private objModel example_model4 = new objModel("smalldiamond.obj");
	
	private float example_rotateT1 = 0.0f;
	private float example_rotateT2 = 0.3f;
	private float example_rotateT3 = 0.5f;


	private float[] LF = {(float) Math.random(),(float)Math.random(),(float)Math.random(),(float)Math.random()};
    private float[] SPEC = {(float) Math.random(),(float)Math.random(),(float)Math.random(),(float)Math.random()};
	...
	public void display(GLAutoDrawable drawable) {
		...
		/* === YOUR WORK HERE === */
		//AESTHETICS TIER 0. LINES AND THE SCULPTURE'S BASE. NO DESCENDANTS.
		gl.glPushMatrix();	
		gl.glScalef(2, 1, 2);	
		gl.glLightfv( GL.GL_LIGHT0, GL.GL_POSITION, LF, 0);
		gl.glLightfv( GL.GL_LIGHT0, GL.GL_SPECULAR, SPEC, 0);
		example_model1.Draw();
		gl.glBegin(GL.GL_LINES);
		  gl.glVertex3f(centerx, centery+1.25f, centerz);
		  gl.glVertex3f(centerx, centery-.75f, centerz);
		gl.glEnd();
		gl.glBegin(GL.GL_LINES);
		  gl.glVertex3f(centerx+.1f, centery+1.25f, centerz);
		  gl.glVertex3f(centerx+.1f, centery-1f, 0);
		gl.glEnd();
		gl.glBegin(GL.GL_LINES);
		  gl.glVertex3f(centerx-.1f, centery+1.25f, centerz);
		  gl.glVertex3f(centerx-.1f, centery-1f, 0);
		...
		// BASE TIER 1. ONE DESCENDANT
		gl.glPushMatrix();
		gl.glTranslatef(0, -1.1f, 0);
		gl.glRotatef(example_rotateT1, 0, 1, 0);
		gl.glLightfv( GL.GL_LIGHT0, GL.GL_POSITION, LF, 0);
		gl.glLightfv( GL.GL_LIGHT0, GL.GL_SPECULAR, SPEC, 0);
		example_model2.Draw();
			gl.glPushMatrix();
			gl.glTranslatef(0, -1.1f, 0);
			gl.glScalef(-0.5f, -0.1f, -0.3f);	
			gl.glRotatef(example_rotateT1, 0, 1, 0);
			example_model4.Draw();
			gl.glPopMatrix();
		gl.glPopMatrix();

		// BASE TIER 2. TWO DESCENDANT
		gl.glPushMatrix();
		gl.glTranslatef(0.2f, -1.7f, 0);
		gl.glRotatef(example_rotateT2, 0, 1, 0);
		...
		// BASE TIER 3. TWO DESCENDANT
		gl.glPushMatrix();
		gl.glTranslatef(-0.2f, -1.7f, 0);
		gl.glRotatef(example_rotateT3, 0, 1, 0);
		gl.glScalef(-0.7f, 1, 1);
		gl.glLightfv( GL.GL_LIGHT0, GL.GL_POSITION, LF, 0);
		example_model3.Draw();
			gl.glPushMatrix();
		...
		/* increment example_rotateT# and randomize SPEC and LF */
		if (animator.isAnimating())
			Arrays.fill(LF, 0, LF.length, (float)Math.random());
			Arrays.fill(SPEC, 0, SPEC.length, (float)Math.random());
			example_rotateT1 += 1.1f * animation_speed;
			example_rotateT2 += 3.1f * animation_speed;
			example_rotateT3 += 2.5f * animation_speed;
	}	
